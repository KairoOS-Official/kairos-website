...
class KairoNavbar extends HTMLElement {
  connectedCallback() {
    ...
    const isSubPage = path.includes('/themes') || path.includes('/plugins') || path.includes('/roadmap') || path.includes('/pages/') || this.getAttribute('root') === '../';
        ...
        <div class="h-16 w-full rounded-2xl bg-white/85 backdrop-blur-xl border border-black/5 shadow-subtle px-4 sm:px-5 flex items-center justify-between gap-3">
          ...
          <a class="flex items-center gap-3 group shrink-0" href="${homeHref}">
            <div class="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center p-1.5 shadow-sm group-hover:scale-105 transition-transform">
              <img alt="KairoOS Logo" class="w-full h-full object-contain filter invert" src="${root}assets/img/logo.png">
            </div>
            <div class="flex flex-col">
              <div class="flex items-center gap-1.5">
                <span class="font-display font-bold text-lg tracking-tight text-heading">Kairo<span class="text-brand-600">OS</span></span>
  ...
  setActive(active, isSubPage = window.location.pathname.includes('/pages/')) {
    ...
    const pageMap = {
      ...
      'Thèmes': `${root}pages/themes.html`,
      'Thèmes & Personnalisation': `${root}pages/themes.html`,
      'Plugins': `${root}pages/plugins.html`,
      'Plugins & Extensions': `${root}pages/plugins.html`,
      'Roadmap': `${root}pages/roadmap.html`,
      'Feuille de route': `${root}pages/roadmap.html`,
      'Home': `${root}index.html`,
      'Themes': `${root}pages/themes.html`,
      'Themes & Personalization': `${root}pages/themes.html`
    };
